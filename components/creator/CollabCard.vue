<script setup lang="ts">
  ...
  const props = defineProps<{ collab: ICampaign, pending:boolean}>();
...
<template>
  <div
    ...
    <div v-if="pending" class="flex gap-2 justify-between items-center pb-2 px-3  border-b border-b-darkBlue">
  ...
  -->
      ...
      <div class="flex justify-between items-center px-2">
        <div class="flex flex-col">
          ...
          <p class="uppercase font-extrabold text-2xl">{{collab.compensation.currency}} {{ collab.compensation.price.toLocaleString() }}</p>
        </div>
