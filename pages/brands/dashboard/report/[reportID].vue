...
const getCampaigns = async (reportID) => {
  try {
    const res = await $fetch<APIResponse<"profiles", IUserProfile[]>>(
      `${API_URL}/campaign/brand/${reportID}/creators`,
      {
...
<template>
  ...
  <div v-else class="print-body px-4 flex flex-col gap-4 h-screen text-white">
    ...
    <section class="tab-section text-white flex w-full">
      ...
      <div class="border-b-grey1 border-b-[1px] w-full"></div>
    </section>

    <!-- Key results section -->
    <div
      v-if="selectedTab === 'Campaign Summary' && campaign"
    ...
    <div v-if="selectedTab === 'Creators'">
      ...
      <div v-else class="mx-4 mt-10">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
          ...
          >
            <thead class="text-xs tracking-tighter bg-darkBlue text-purplebg">
              <tr>
                <th scope="col" class="py-3 text-left px-6 text-[#CDC2FF]">
                  Creators
                </th>
    ...
    <div v-if="selectedTab === 'Content'">
      ...
      <div class="grid md:grid-cols-4 grid-cols-2 gap-8">
        <div
          v-for="sample in CampaignResults"
          :key="sample.id"
          class="bg-[#090618] flex justify-between rounded-lg"
        >
          <Dialog>
            <DialogTrigger class="w-fit cursor-pointer">
              <div class="hover:grayscale-0 grayscale w-fit">
                <img
                  src="/assets/icons/creatorContent.svg"
                  alt=""
                  class="w-full"
                />
                <div class="p-4">
            ...
            <DialogContent
              class="bg-[#090618] max-w-[300px] text-white border-none"
            >
              <DialogHeader>
                <DialogTitle>Post Details</DialogTitle>
                <div class="w-full flex justify-center py-4">
                  <button
                    class="rounded-[20px] bg-black max-w-fit p-2"
                    @click="external(sample.url)"
                  >
                    view live post
                  </button>
                </div>
                <div class="text-center flex flex-col gap-4">
                  <span
                    class="flex border-b-[0.5px] border-b-[1D192F] justify-between"
                  >
                    <p>Comments :</p>
                  ...
                  >
                    <p>Likes:</p>
                    <p>{{ sample.likeCount }}</p></span
                  >
                  <span
                    class="flex border-b-[0.5px] border-b-[1D192F] justify-between"
                  >
                    <p>Shares:</p>
                    <p>{{ sample.shareCount }}</p></span
                  >
                  <span
                    class="flex border-b-[0.5px] border-b-[1D192F] justify-between"
                  >
                    <p>Views:</p>
                    <p>{{ sample.viewCount }}</p></span
                  >
                  <span
                    class="flex border-b-[0.5px] border-b-[1D192F] justify-between"
                  >
                    ...
                    {{
                      (
                        (sample.commentCount +
                          sample.likeCount +
                          sample.shareCount) /
                        sample.platformProfile.reputationFollowerCount
                      ).toFixed(2)
                    }}%
                  </span>
                </div>
